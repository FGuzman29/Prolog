...
regla():- monedas(Lista),contar(Lista,Cant),write(Cant),nl,Cant == 4,!,fail. 
...
%Parte modificada:
%Observamos cuando hicimos las diferentes consultas que este programa no funciona correctamente ya
%que no verifique el N1 y N2 sean de tipo number por lo que al comparar una variable con un
%numero este daba error. Para corregir este problema lo único que hicimos fue verificar que 
